...
export default function Quiz() {
  ...
  const [attempts, setAttempts] = useState(
    new Array(quizObject.questions.length).fill(undefined)
  );
  ...
  const calculateScore = () => {
    return quizObject.answers.reduce(
      (acc, curr, index) => (curr === attempts[index] ? acc + 20 : acc),
      0
    );
  };
